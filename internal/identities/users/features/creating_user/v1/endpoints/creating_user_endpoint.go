...
func MapRoute(userManager data.IUserManager, jwt jwt.IJwtTokenValidator, checker permissions.IPermissionChecker, log logger.ILogger, echo *echo.Echo, ctx context.Context) {
	group := echo.Group("/api/v1/user")
	group.POST("", createUser(userManager, log, ctx), middlewares.ValidateToken(jwt), middlewares.Authorize(checker, permissions.PagesAdministrationUsersCreate))
}
...
func createUser(userManager data.IUserManager, log logger.ILogger, ctx context.Context) echo.HandlerFunc {
